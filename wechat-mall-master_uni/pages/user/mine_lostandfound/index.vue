	...
	<block>
		<view class="cu-list goods">
			<view class="cu-item bg-white" v-for="(item, index) in list" :key="item.id">
				<navigator class="content" :url="'../../add_detail/Lostandfound/detail?id=' + item.id">
				...
				<view class="flex align-end">
					<view class="action text-xl flex align-center">
					    <navigator class="margin-lr-lg" :url="'../../add_detail/Lostandfound/edit?id=' + item.id">
					...
					success: async ({
						...
						if (confirm) {
							let {
								status,
								msg
							} = await User.cat_off_lostandfound(id);
