  ...
  <div>
    <el-card class="box-card">
      ...
      <el-table :data="tableData" style="width:100%">
        <el-table-column fixed label="求购名称" width="550">
        ...
        <el-table-column sortable prop="create_time" label="发布时间">
                ...
                <el-table-column sortable prop="create_name" label="发布人">
        </el-table-column>
...
export default {
  ...
  methods: {
    async loadGoodsList() {
      let { status, list, total } = await buy.list({
      });
      if (status) {
        this.tableData = list;
        this.total = total;
    ...
    async showDeleteModal(id) {
      this.$confirm('确定要删除该商品吗？删除之后无法恢复！！！', {
        type: 'warning'
          ...
          .then(async () => {
            let { status, msg } = await buy.remove(id);
            if (status) {
